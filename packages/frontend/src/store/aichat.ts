import { APIProvider, SelectedLLM } from '@prisma-ai/shared';
import { createSlice, type PayloadAction } from '@reduxjs/toolkit';
import { selectedLLMToApiType } from '../services/apikey/apikey_ls.service';

// AI聊天模块的状态接口
export interface AIChatState {
	model: SelectedLLM;
	apiType: APIProvider;
	lastConversation: string; // 上一次对话的keyname，用于恢复对话
}

// 初始化状态
const initialState: AIChatState = {
	model: SelectedLLM.glm4_5,
	apiType: APIProvider.zhipu,
	lastConversation: ''
};

// 创建 AI聊天 的 Redux Slice
const AIChatSlice = createSlice({
	name: 'aichat',
	initialState,
	reducers: {
		// 设置选中的LLM模型
		setAIChatLLM: (state, { payload }: PayloadAction<SelectedLLM>) => {
			state.model = payload;
			state.apiType = selectedLLMToApiType[payload];
		},
		setAIChatLastConversation: (state, { payload }: PayloadAction<string>) => {
			state.lastConversation = payload;
		}
	}
});

// 导出 Actions
export const { setAIChatLLM, setAIChatLastConversation } = AIChatSlice.actions;

// 导出 Selector，用于从 store 中获取 AI聊天 相关数据
export const selectAIChatLLM = (state: { aichat: AIChatState }) => state.aichat.model;
export const selectAIChatApiType = (state: { aichat: AIChatState }) => state.aichat.apiType;
export const selectAIChatLastConversation = (state: { aichat: AIChatState }) =>
	state.aichat.lastConversation;

// 导出 Reducer，用于在 store 中注册
export const AIChatReducer = AIChatSlice.reducer;
