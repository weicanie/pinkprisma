...
import type { APIProvider, ChatMessage, ConversationDto, SelectedLLM } from '@prisma-ai/shared';
...
const AIChat: React.FC<AIChatProps> = ({ className }) => {
	...
	// ==================== Event ====================
	const apiType = useSelector(selectAIChatApiType);
	...
	const onSubmit = async (val: string) => {
		...
		try {
			const res = await sendMessageToAI<SelectedLLM>(userMessage, curConversation, {
				api_type: apiType,
				llm_type: model,
				apiKey: keyEntity.data.apiKey
			});
			...
			{messages?.length ? (
				...
				<div className="pt-8 w-full flex flex-col sm:flex-row gap-5 sm:justify-center items-center">
					<Prompts
						...
						onItemClick={info => {
							//@ts-ignore
							onSubmit(info.data.prompt as string);
						}}
						className=""
					/>
